...
This service is **read-only**. It publishes governed offboarding data to authorized consumer systems. It does not disable accounts, remove permissions, execute application-specific offboarding steps, or persist the downstream offboarding workflow. ACBS, Workiva, PeopleSoft, and future consumers remain responsible for their own internal deprovisioning logic.

esource-definitions.json is the configuration-driven exposure model: each public resource declares a logical model, contractVersion, approved Databricks view, public field mappings, filter/sort capabilities, defaults, and page limits.
...
- [Final Gap Assessment](docs/GAP_ASSESSMENT.md)
